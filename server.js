...
io.on('connection', (socket) => {
    ...
    // --- FOLLOW / UNFOLLOW SİSTEMİ (Düzəldildi) ---
    socket.on('follow_user', async (data) => {
        ...
        if (me && target && current_user !== target_user && !me.following.includes(target_user)) {
            ...
            await me.save();
            await target.save();
            io.emit('follow_updated', { target_user, followersCount: target.followers.length });
        }
    });
    ...
    // --- AVATAR (PROFİL ŞƏKLİ) YENİLƏMƏSİ (Əlavə edildi!) ---
